...
public class SmartCircularMenuItem {
	...
	private OnSmartCircularMenuPressed mCallback;
	private int mIconDimen;

	public interface OnSmartCircularMenuPressed {
	 ...
	 * @param mText
	 */
	public SmartCircularMenuItem(String id, Drawable mIcon, String mText) {
		super();
		this.mMenuID = id;
		this.mIcon = mIcon;
		this.mText = mText;
		this.mMenuNormalColor = Color.BLACK;
		this.mMenuSelectedColor = SmartCircularMenuColors.HOLO_LIGHT_BLUE;
	...
	public void setMenuPath(RectF menuButtonRect, RectF menuRect,
			float StartArc, float StartArcCenter, float ArcWidth, float radius,
			Point anchorPoint, boolean isBottom) {
		int left, right, top, bottom;
		left = right = top = bottom = 0;
		// Draw the widget path
		mPath.arcTo(menuRect, StartArc, ArcWidth);
		if (isBottom)
			mPath.arcTo(menuButtonRect,
					StartArcCenter + (int) (ArcWidth * 1.5),
					-(int) (ArcWidth * 1.5));
		else
	 ...
	 */
	public void setOnSmartCircularMenuPressed(
			OnSmartCircularMenuPressed mCallback) {
	 ...
	 */
	public OnSmartCircularMenuPressed getCallback() {
		return mCallback;
	}
